...
export async function POST(request: Request) {
  ...
  if (action === "magiclink") {
    ...
    const { error } = await createDoc2PostdocEmailClient().auth.signInWithOtp({ email, options: { emailRedirectTo: `${origin}/verify-credential?kind=doc2postdoc`, shouldCreateUser: true, data: { display_name: displayName } } });
  ...
  const result = action === "signup"
    ? await createDoc2PostdocEmailClient().auth.signUp({ email, password, options: { data: { display_name: displayName } } })
